    ...
    for c in data["controllers"]:
        ...
        for bat in data["battery_modules"]:
            if kb["split"] == "true":
                ...
                if kb.get("promicro") == "true":
                    s_left_base = [f"halcyon_to_promicro {kb['name']}_left", bat["name"]]
                    s_right_base = [f"halcyon_to_promicro {kb['name']}_right", bat["name"]]
